...
@section('title', 'Template Komponen Gaji')
@section('page-title', 'Template Komponen Gaji')
@section('page-subtitle', 'Atur komponen gaji per project, jabatan, atau karyawan')
...
<div class="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-6">
    <div class="flex items-start gap-3">
        ...
        <div class="flex-1">
            <h3 class="text-sm font-bold text-blue-900 mb-2">Template Komponen Per Jabatan</h3>
            <div class="space-y-1 text-xs text-blue-800">
                <div class="flex items-start gap-2">
                    <i class="fas fa-building text-blue-600 mt-0.5"></i>
                    <span><strong>Level 1 - Default:</strong> Template dasar untuk semua karyawan (Gaji Pokok + BPJS - Potongan - Pajak)</span>
                </div>
                <div class="flex items-start gap-2">
                    <i class="fas fa-briefcase text-blue-600 mt-0.5"></i>
                    <span><strong>Level 2 - Jabatan:</strong> Override template default untuk jabatan tertentu (contoh: Security dapat + Uang Makan)</span>
                </div>
            </div>
            <div class="mt-3 pt-3 border-t border-blue-300">
                <div class="flex items-start gap-2 text-xs text-blue-800">
                    ...
                    <span><strong>Contoh:</strong> Semua karyawan dapat Rp 5jt. Security Officer dapat Rp 5jt + Uang Makan Rp 25rb</span>
                </div>
            </div>
        </div>
    </div>
...
<div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
    <form method="GET" action="{{ route('perusahaan.template-komponen.index') }}" class="space-y-4">
        ...
        <div class="flex items-start gap-4">
            ...
            <div class="flex-1">
                ...
                <p class="text-sm text-gray-700 mb-3">
                    Template dasar yang berlaku untuk <strong>semua karyawan</strong> di setiap project. 
                ...
                <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    ...
                    <div class="space-y-2 text-xs text-blue-800">
                        <div class="flex items-start gap-2">
                            ...
                            <span>Sistem otomatis hitung gaji pokok + BPJS - potongan</span>
                        </div>
                        <div class="flex items-start gap-2">
                            <span class="font-bold text-blue-600">2.</span>
                            <span>Jika ada template jabatan, komponen ditambahkan</span>
                        </div>
                        <div class="flex items-start gap-2">
                            <span class="font-bold text-blue-600">3.</span>
                            <span>Jika ada template karyawan, nilai di-override</span>
                        </div>
                    </div>
                    <div class="mt-3 pt-3 border-t border-blue-300">
                        <p class="text-xs text-blue-700">
                            <i class="fas fa-lightbulb mr-1"></i>
                            <strong>Tip:</strong> Buat template kustom hanya untuk yang berbeda dari default
                        </p>
                    </div>
                </div>
            </div>
...
<div class="mb-4">
    <h2 class="text-lg font-bold text-gray-900 flex items-center gap-2">
        ...
        Template Kustom (Override)
    </h2>
    <p class="text-sm text-gray-600 mt-1">Template khusus untuk jabatan atau karyawan tertentu</p>
</div>

@forelse($groupedByProject as $projectId => $projectTemplates)
    @php
        $project = $projectTemplates->first()->project;
        $totalTemplates = $projectTemplates->groupBy('nama_template')->count();
    @endphp
        ...
        <div class="flex items-center justify-between text-white">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-sm text-blue-100">{{ $totalTemplates }} template kustom</p>
                </div>
            </div>
    ...
    <div id="project-{{ $projectId }}" class="mb-6">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
            @foreach($projectTemplates->groupBy('nama_template') as $namaTemplate => $templateList)
                @php
                    $firstTemplate = $templateList->first();
                    $deskripsi = $firstTemplate->deskripsi;
                    $level = $firstTemplate->level;
                    $target = '';
                    if ($firstTemplate->karyawan) {
                        $target = $firstTemplate->karyawan->nama_lengkap;
                    } elseif ($firstTemplate->jabatan) {
                        $target = $firstTemplate->jabatan->nama;
                    } else {
            ...
            <div class="p-4 border-b border-gray-200">
                <div class="flex items-start justify-between mb-2">
                    <h3 class="text-lg font-bold text-gray-900 flex-1">{{ $namaTemplate ?: 'Template Tanpa Nama' }}</h3>
                    <div class="flex items-center gap-2">
                        <button onclick="editTemplate('{{ $namaTemplate }}', '{{ $project->id }}')" class="text-blue-600 hover:text-blue-800" title="Edit Template">
                            <i class="fas fa-edit"></i>
                        </button>
                        <button onclick="deleteTemplate('{{ $namaTemplate }}')" class="text-red-600 hover:text-red-800" title="Hapus Template">
                ...
                @if($deskripsi)
                    <p class="text-sm text-gray-600 mt-2">{{ Str::limit($deskripsi, 60) }}</p>
                @endif
            </div>

            <!-- Card Body -->
            <div class="p-4 space-y-3">
                <!-- Jabatan Info (Primary) -->
                <div class="flex items-center gap-2 text-sm">
                    <i class="fas fa-briefcase text-gray-400 w-4"></i>
                    <span class="text-gray-600">Jabatan:</span>
                    <span class="font-medium text-gray-900">
                        @if($firstTemplate->jabatan)
                            {{ $firstTemplate->jabatan->nama }}
                        @else
                            -
                        @endif
                    </span>
                ...
                <div class="flex items-center gap-2 text-sm">
                    ...
                    <span class="font-medium text-gray-900">{{ Str::limit($target, 20) }}</span>
                </div>
                ...
                <div class="bg-blue-50 border border-blue-200 rounded-lg p-2 mb-3">
                    <p class="text-xs text-blue-800 flex items-start gap-1">
                        ...
                        <span><strong>Catatan:</strong> Template default (Gaji Pokok + BPJS - Potongan) tetap berlaku. Komponen di bawah adalah <strong>tambahan</strong>.</span>
                    </p>
                </div>
...
@empty
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
        <div class="w-24 h-24 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-4">
            <i class="fas fa-puzzle-piece text-gray-400 text-4xl"></i>
        </div>
        <h3 class="text-xl font-bold text-gray-900 mb-2">Belum Ada Template</h3>
        <p class="text-gray-600 mb-4">Klik tombol "Tambah" untuk membuat template komponen gaji</p>
    </div>
@endforelse
...
<div id="modalForm" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
    <div class="bg-white rounded-2xl shadow-2xl max-w-3xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        <form id="templateForm" method="POST" action="{{ route('perusahaan.template-komponen.store') }}">
            @csrf
            
            <!-- Modal Header -->
            <div class="px-6 py-4 border-b border-gray-200">
                <h3 class="text-xl font-bold text-gray-900">Tambah Template Komponen</h3>
                <p class="text-sm text-gray-600 mt-1">Atur komponen gaji untuk jabatan tertentu</p>
            </div>
            ...
            <div class="p-6 space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="nama_template" id="nama_template" placeholder="Contoh: Template A" 
                ...
                <div>
                    ...
                    <div class="flex items-center gap-2">
                        <select name="project_id" id="project_id" class="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" required onchange="loadJabatans()">
                    ...
                    <p class="text-xs text-gray-500 mt-1">Project digunakan untuk filter jabatan</p>
                </div>
                ...
                <div>
                    ...
                    <select name="jabatan_id" id="jabatan_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" required>
...
function openModal(mode, hashId = null) {
    ...
    // Auto-load jabatans if project is already selected
...
function formatNumber(num) {
    // Convert to number first to handle decimal values
...
function loadJabatans(excludeJabatanId = null) {
    ...
    return fetch(`/perusahaan/jabatans/by-project/${projectId}`)
        ...
        .then(jabatans => {
            // Get used jabatans and disable them
            let url = `/perusahaan/template-komponen/get-used-jabatans?project_id=${projectId}`;
            if (excludeJabatanId) {
                url += `&exclude_jabatan_id=${excludeJabatanId}`;
            }
            ...
            return fetch(url)
                ...
                .then(usedJabatanIds => {
                    ...
                    jabatans.forEach(jabatan => {
                        const isUsed = usedJabatanIds.includes(jabatan.id);
                        const option = document.createElement('option');
                        option.value = jabatan.id;
                        option.textContent = jabatan.nama + (isUsed ? ' (Sudah ada template)' : '');
...
function deleteTemplate(namaTemplate) {
    // Handle null or empty nama_template
    const displayName = namaTemplate || 'Template Tanpa Nama';
    const actualValue = namaTemplate || '';
    
    Swal.fire({
        title: 'Yakin ingin menghapus?',
        html: `Template <strong>"${displayName}"</strong> dan semua komponennya akan dihapus permanen!`,
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            ...
            form.method = 'POST';
            form.action = '{{ route("perusahaan.template-komponen.destroy-by-name") }}';
            ...
            const namaTemplateInput = document.createElement('input');
            namaTemplateInput.type = 'hidden';
            namaTemplateInput.name = 'nama_template';
            namaTemplateInput.value = actualValue;
            
            form.appendChild(csrfToken);
            form.appendChild(namaTemplateInput);
...
function editTemplate(namaTemplate, projectId) {
    // Fetch template data
    fetch(`/perusahaan/template-komponen/get-by-name?nama_template=${encodeURIComponent(namaTemplate)}&project_id=${projectId}`)
        .then(response => response.json())
        .then(data => {
            if (data.error) {
                Swal.fire({
            ...
            // Load jabatans with exclude current jabatan (for edit mode)
            loadJabatans(data.jabatan_id).then(() => {
                if (data.jabatan_id) {
