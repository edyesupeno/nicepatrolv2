...
@section('page-subtitle', 'Kelola jabatan untuk semua project')
    ...
    <div class="flex items-center gap-4">
        <div class="bg-white px-6 py-3 rounded-xl shadow-sm border border-gray-100">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-xs text-gray-500 font-medium">Total Jabatan</p>
                    <p class="text-2xl font-bold" style="color: #3B82C8;">{{ $jabatans->total() }}</p>
                </div>
            </div>
        </div>
    </div>
...
<div class="mb-6 bg-white rounded-xl shadow-sm border border-gray-100 p-4">
    <form method="GET" class="flex flex-col lg:flex-row gap-3">
        ...
        <div class="flex gap-2">
            ...
            <a 
                href="{{ route('perusahaan.jabatans.index') }}"
            ...
            <thead class="bg-gradient-to-r from-gray-50 to-gray-100">
                <tr>
                    <th class="px-6 py-4 text-left text-xs font-bold text-gray-600 uppercase tracking-wider">
                        <i class="fas fa-user-tie mr-2" style="color: #3B82C8;"></i>Nama Jabatan
                    </th>
                    <th class="px-6 py-4 text-left text-xs font-bold text-gray-600 uppercase tracking-wider">
                ...
                <tr class="hover:bg-blue-50 transition-colors duration-150">
                    <td class="px-6 py-4">
                        <div class="flex items-center">
                            ...
                            <div>
                                <p class="text-sm font-semibold text-gray-900">{{ $jabatan->nama }}</p>
                                <p class="text-xs text-gray-500">ID: {{ $jabatan->hash_id }}</p>
                            </div>
                        </div>
                    </td>
                    <td class="px-6 py-4">
                        <div class="flex flex-wrap gap-1">
                            @forelse($jabatan->projects as $project)
                        ...
                        <p class="text-xs text-gray-500 mt-1">{{ $jabatan->projects->count() }} project</p>
                    </td>
                    <td class="px-6 py-4">
                        <p class="text-sm text-gray-600">{{ $jabatan->deskripsi ?? '-' }}</p>
                    </td>
                    <td class="px-6 py-4">
                        <div class="flex items-center justify-center gap-2">
                            <button 
                                onclick="openEditModal('{{ $jabatan->hash_id }}')" 
                            ...
                            <button 
                                onclick="confirmDelete('{{ $jabatan->hash_id }}')" 
                    ...
                    <td colspan="4" class="px-6 py-16 text-center">
                        <div class="flex flex-col items-center justify-center">
                            <div class="w-24 h-24 rounded-full flex items-center justify-center mb-4" style="background: linear-gradient(135deg, #E0F2FE 0%, #BAE6FD 100%);">
                                <i class="fas fa-user-tie text-5xl" style="color: #3B82C8;"></i>
                            </div>
                            ...
                            <p class="text-gray-500 text-sm mb-6">
                                ...
                                @else
                                    Tambahkan jabatan pertama Anda untuk memulai
                                @endif
                            </p>
                            @if(request('search') || request('project_id'))
                                <a 
...
<div id="modalCreate" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
        <form action="{{ route('perusahaan.jabatans.store') }}" method="POST" id="formCreate">
            @csrf
            <!-- Modal Header -->
            <div class="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 rounded-t-2xl">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h3 class="text-xl font-bold text-gray-900">Tambah Jabatan Baru</h3>
                            <p class="text-sm text-gray-500">Isi form di bawah untuk menambah jabatan</p>
                        </div>
                    </div>
            ...
            <div class="p-6 space-y-5">
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">
                        <i class="fas fa-user-tie mr-2" style="color: #3B82C8;"></i>Nama Jabatan <span class="text-red-500">*</span>
                    </label>
                    <input 
                        type="text" 
                        name="nama" 
                        ...
                        style="focus:ring-color: #3B82C8;"
                        placeholder="Contoh: Direktur, Manager, Supervisor, dll"
                    >
                    <p class="mt-1 text-xs text-gray-500">Masukkan nama jabatan yang jelas</p>
                </div>
                ...
                <div>
                    ...
                    ></textarea>
                    <p class="mt-1 text-xs text-gray-500">Deskripsi singkat tentang jabatan ini</p>
                </div>
                    ...
                    <div class="mb-3 p-3 bg-blue-50 rounded-lg border border-blue-200">
                        <label class="flex items-center cursor-pointer">
                            <input 
                                type="checkbox" 
                                id="selectAllCreate"
                                onchange="toggleAllProjects(this, 'create')"
                    ...
                    <div class="space-y-2 max-h-60 overflow-y-auto border border-gray-200 rounded-xl p-4">
                        ...
                        <label class="flex items-center p-3 hover:bg-gray-50 rounded-lg cursor-pointer transition">
                            <input 
                                ...
                                name="projects[]" 
                                value="{{ $project->id }}"
                                class="project-checkbox-create w-5 h-5 text-blue-600 border-gray-300 rounded focus:ring-2 focus:ring-blue-500"
                                onchange="updateSelectAll('create')"
                            >
                            <span class="ml-3 text-sm text-gray-700">{{ $project->nama }}</span>
                        </label>
                        @empty
                        <p class="text-sm text-gray-500 text-center py-4">Belum ada project aktif</p>
                        @endforelse
                    </div>
                    <p class="mt-2 text-xs text-gray-500">Pilih minimal 1 project untuk jabatan ini</p>
                </div>
            </div>
            ...
            <div class="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 rounded-t-2xl">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h3 class="text-xl font-bold text-gray-900">Edit Jabatan</h3>
                            <p class="text-sm text-gray-500">Update informasi jabatan</p>
                        </div>
                    </div>
...
async function openEditModal(hashId) {
    try {
        const response = await fetch(`/perusahaan/jabatans/${hashId}/edit`);
        const data = await response.json();
        
        document.getElementById('edit_nama').value = data.nama;
        document.getElementById('edit_deskripsi').value = data.deskripsi || '';
        document.getElementById('formEdit').action = `/perusahaan/jabatans/${hashId}`;
...
function confirmDelete(hashId) {
    Swal.fire({
        ...
        text: "Data jabatan akan dihapus dari semua project!",
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            ...
            form.action = `/perusahaan/jabatans/${hashId}`;
            form.submit();
        }
    });
}
