...
<div class="container-fluid px-4 py-6">
    ...
    <div class="mb-6">
        <div class="flex items-center gap-3 mb-3">
            ...
            <div>
                <h1 class="text-3xl font-bold text-gray-900">{{ $kuesionerPatroli->judul }}</h1>
                <p class="text-gray-600">{{ $kuesionerPatroli->deskripsi }}</p>
            </div>
        </div>
        
        <div class="flex items-center gap-3">
            <span class="px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-sm font-medium">
                <i class="fas fa-list mr-1"></i>{{ $kuesionerPatroli->pertanyaans->count() }} Pertanyaan
            </span>
            @if($kuesionerPatroli->is_active)
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-xl shadow-sm">
                <div class="p-6 border-b border-gray-200 flex items-center justify-between">
                    ...
                    <a 
                        href="{{ route('perusahaan.patrol.kuesioner-patroli.preview', $kuesionerPatroli->hash_id) }}"
                        target="_blank"
...
function updateUrutan() {
    ...
    // Send to server
    fetch('{{ route("perusahaan.patrol.kuesioner-patroli.urutan", $kuesionerPatroli->hash_id) }}', {
        method: 'POST',
        headers: {
...
document.getElementById('pertanyaanForm').addEventListener('submit', function(e) {
    ...
    let url = '{{ route("perusahaan.patrol.kuesioner-patroli.pertanyaan.store", $kuesionerPatroli->hash_id) }}';
    
    if (method === 'PUT' && pertanyaanId) {
        url = `{{ url('perusahaan/patrol/kuesioner-patroli/' . $kuesionerPatroli->hash_id . '/pertanyaan') }}/${pertanyaanId}`;
...
function editPertanyaan(id) {
    // Get pertanyaan data
    const pertanyaanData = @json($kuesionerPatroli->pertanyaans);
...
function deletePertanyaan(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            form.action = `{{ url('perusahaan/patrol/kuesioner-patroli/' . $kuesionerPatroli->hash_id . '/pertanyaan') }}/${id}`;
            form.submit();
        }
    });
}
