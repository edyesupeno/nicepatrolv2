...
<head>
    ...
    <title>{{ $pemeriksaanPatroli->nama }}</title>
    ...
    <div class="min-h-screen flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl shadow-2xl max-w-lg w-full">
            ...
            <div class="bg-gradient-to-r from-blue-600 to-blue-800 px-6 py-8 text-white text-center rounded-t-2xl">
                ...
                <h1 class="text-2xl font-bold mb-2">{{ $pemeriksaanPatroli->nama }}</h1>
                <p class="text-blue-100 text-sm">{{ $pemeriksaanPatroli->deskripsi }}</p>
                <div class="mt-3">
                    @if($pemeriksaanPatroli->frekuensi === 'harian')
                        <span class="px-3 py-1 bg-white bg-opacity-20 rounded-full text-xs font-medium">
                            <i class="fas fa-calendar-day mr-1"></i>Pemeriksaan Harian
                        </span>
                    @elseif($pemeriksaanPatroli->frekuensi === 'mingguan')
                        <span class="px-3 py-1 bg-white bg-opacity-20 rounded-full text-xs font-medium">
                            <i class="fas fa-calendar-week mr-1"></i>Pemeriksaan Mingguan
                        </span>
                    @else
                        <span class="px-3 py-1 bg-white bg-opacity-20 rounded-full text-xs font-medium">
                            <i class="fas fa-calendar-alt mr-1"></i>Pemeriksaan Bulanan
                        </span>
                    @endif
                </div>
            </div>
            ...
            <div class="p-6">
                ...
                @else
                    <div class="text-center py-12">
                        ...
                        <p class="text-gray-500">Belum ada pertanyaan dalam pemeriksaan ini</p>
                    </div>
                @endif
            </div>
            ...
            <div class="bg-gray-50 px-6 py-4 text-center border-t rounded-b-2xl">
                <p class="text-xs text-gray-500">
                    ...
                    Ini adalah preview pemeriksaan. Tombol tidak berfungsi.
                </p>
            </div>
        </div>
    </div>
