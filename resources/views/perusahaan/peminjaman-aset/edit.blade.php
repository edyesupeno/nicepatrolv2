...
<div class="mb-6">
    <div class="flex items-center space-x-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Edit Peminjaman Aset</h1>
...
<div class="bg-white rounded-lg shadow-sm border border-gray-200">
    <form action="{{ route('perusahaan.peminjaman-aset.update', $peminjamanAset->hash_id) }}" method="POST" enctype="multipart/form-data" class="p-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <select name="project_id" id="project_id" required class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                        ...
                        @foreach($projects as $project)
                            <option value="{{ $project->id }}" {{ old('project_id', $peminjamanAset->project_id) == $project->id ? 'selected' : '' }}>
                                {{ $project->nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div id="aset-selection">
                    ...
                    <input type="hidden" name="data_aset_id" id="data_aset_id" value="{{ old('data_aset_id', $peminjamanAset->data_aset_id) }}">
                    <input type="hidden" name="aset_kendaraan_id" id="aset_kendaraan_id" value="{{ old('aset_kendaraan_id', $peminjamanAset->aset_kendaraan_id) }}">
                    ...
                    <!-- Aset Info Display -->
                    <div id="aset-info" class="mt-3 p-3 bg-gray-50 rounded-lg {{ $peminjamanAset->aset_nama ? '' : 'hidden' }}">
                        <div class="grid grid-cols-2 gap-4 text-sm">
                            <div>
                                <span class="font-medium text-gray-600">Kode:</span>
                                <span id="aset-kode" class="text-gray-900">{{ $peminjamanAset->aset_kode }}</span>
                            </div>
                            <div>
                                <span class="font-medium text-gray-600">Kategori:</span>
                                <span id="aset-kategori" class="text-gray-900">{{ $peminjamanAset->aset_kategori }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="text" 
                               ...
                               autocomplete="off"
                               value="{{ $peminjamanAset->peminjam_nama }}">
                        <div class="absolute inset-y-0 right-0 flex items-center pr-3">
                            <i class="fas fa-search text-gray-400"></i>
                        </div>
                    </div>
                    ...
                    <input type="hidden" name="peminjam_karyawan_id" id="peminjam_karyawan_id" value="{{ old('peminjam_karyawan_id', $peminjamanAset->peminjam_karyawan_id) }}">
                ...
                <div>
                    ...
                    <input type="number" name="jumlah_dipinjam" id="jumlah_dipinjam" value="{{ old('jumlah_dipinjam', $peminjamanAset->jumlah_dipinjam) }}" min="1" max="100" required class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="date" name="tanggal_peminjaman" id="tanggal_peminjaman" value="{{ old('tanggal_peminjaman', $peminjamanAset->tanggal_peminjaman->format('Y-m-d')) }}" required class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                ...
                <div>
                    ...
                    <input type="date" name="tanggal_rencana_kembali" id="tanggal_rencana_kembali" value="{{ old('tanggal_rencana_kembali', $peminjamanAset->tanggal_rencana_kembali->format('Y-m-d')) }}" required class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                ...
                <div>
                    ...
                    <select name="kondisi_saat_dipinjam" id="kondisi_saat_dipinjam" required class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                        @foreach($kondisiOptions as $value => $label)
                            <option value="{{ $value }}" {{ old('kondisi_saat_dipinjam', $peminjamanAset->kondisi_saat_dipinjam) == $value ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <textarea name="keperluan" id="keperluan" rows="3" required class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="Jelaskan keperluan peminjaman aset...">{{ old('keperluan', $peminjamanAset->keperluan) }}</textarea>
                    @error('keperluan')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea name="catatan_peminjaman" id="catatan_peminjaman" rows="3" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="Catatan tambahan (opsional)...">{{ old('catatan_peminjaman', $peminjamanAset->catatan_peminjaman) }}</textarea>
                ...
                <div>
                    ...
                    @if($peminjamanAset->file_bukti_peminjaman)
                        <div class="mb-2">
                            <a href="{{ $peminjamanAset->file_bukti_peminjaman_url }}" target="_blank" class="inline-flex items-center text-blue-600 hover:text-blue-800 text-sm">
                    ...
                    <p class="text-xs text-gray-500 mt-1">Format: JPG, PNG, PDF. Maksimal 5MB. Kosongkan jika tidak ingin mengubah file.</p>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize asset search based on current values
    if (asetTypeSelect.value && projectSelect.value) {
