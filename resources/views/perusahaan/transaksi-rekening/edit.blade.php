...
@section('title', 'Edit Transaksi - ' . $transaksiRekening->nomor_transaksi)
@section('page-title', 'Edit Transaksi')
@section('page-subtitle', 'Ubah data transaksi rekening')
...
<div class="space-y-6">
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li>
                <div class="flex items-center">
                    ...
                    <a href="{{ route('perusahaan.keuangan.transaksi-rekening.show', $transaksiRekening->hash_id) }}" class="text-sm font-medium text-gray-700 hover:text-blue-600">
    ...
    <div class="flex justify-end">
        <a href="{{ route('perusahaan.keuangan.transaksi-rekening.show', $transaksiRekening->hash_id) }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
    ...
    <div class="max-w-4xl mx-auto">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200">
            <div class="px-6 py-4 border-b border-gray-200 bg-yellow-50">
                ...
                <p class="text-sm text-gray-600 mt-1">{{ $transaksiRekening->nomor_transaksi }}</p>
            </div>
            <div class="p-6">
                <form action="{{ route('perusahaan.keuangan.transaksi-rekening.update', $transaksiRekening->hash_id) }}" method="POST" id="transactionForm">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <select class="w-full px-4 py-4 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('rekening_id') border-red-300 @enderror" 
                                ...
                                @foreach($rekenings as $rekening)
                                    <option value="{{ $rekening->id }}" 
                                            ...
                                            {{ old('rekening_id', $transaksiRekening->rekening_id) == $rekening->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <input type="date" class="w-full px-4 py-4 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('tanggal_transaksi') border-red-300 @enderror" 
                                   name="tanggal_transaksi" value="{{ old('tanggal_transaksi', $transaksiRekening->tanggal_transaksi ? $transaksiRekening->tanggal_transaksi->format('Y-m-d') : '') }}" required>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                        ...
                        <div>
                            ...
                            <select class="w-full px-4 py-4 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('jenis_transaksi') border-red-300 @enderror" 
                                ...
                                <option value="debit" {{ old('jenis_transaksi', $transaksiRekening->jenis_transaksi) === 'debit' ? 'selected' : '' }}>
                                    Debit (Uang Masuk)
                                </option>
                                <option value="kredit" {{ old('jenis_transaksi', $transaksiRekening->jenis_transaksi) === 'kredit' ? 'selected' : '' }}>
                                    Kredit (Uang Keluar)
                                </option>
                            </select>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="text" class="w-full pl-12 pr-4 py-4 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('jumlah') border-red-300 @enderror" 
                                       name="jumlah_display" id="jumlah_display" value="{{ old('jumlah', $transaksiRekening->jumlah) ? number_format(old('jumlah', $transaksiRekening->jumlah), 0, ',', '.') : '' }}" 
                                       placeholder="0" required>
                                <input type="hidden" name="jumlah" id="jumlah" value="{{ old('jumlah', $transaksiRekening->jumlah) }}">
                            </div>
                            @error('jumlah')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="mt-6">
                        ...
                        <div class="relative">
                            <input type="text" 
                                   ...
                                   value="{{ old('kategori_transaksi', $transaksiRekening->kategori_transaksi) }}"
                                   placeholder="Ketik untuk mencari atau tambah kategori baru..."
                                   autocomplete="off"
                                   required>
                            <input type="hidden" name="kategori_transaksi" id="kategori_transaksi_hidden" value="{{ old('kategori_transaksi', $transaksiRekening->kategori_transaksi) }}">
                    ...
                    <div class="mt-6">
                        ...
                        <textarea class="w-full px-4 py-4 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('keterangan') border-red-300 @enderror" 
                                  name="keterangan" rows="4" placeholder="Masukkan keterangan transaksi..." required>{{ old('keterangan', $transaksiRekening->keterangan) }}</textarea>
                        @error('keterangan')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mt-6">
                        ...
                        <input type="text" class="w-full px-4 py-4 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('referensi') border-red-300 @enderror" 
                               name="referensi" value="{{ old('referensi', $transaksiRekening->referensi) }}" 
                            ...
                            <div class="flex">
                                ...
                                <div class="ml-3">
                                    ...
                                    <div class="mt-2 text-sm text-yellow-700">
                                        <p>Transaksi yang sudah diverifikasi tidak dapat diedit. Perubahan akan mempengaruhi saldo rekening.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="flex justify-end space-x-3 mt-8">
                        <a href="{{ route('perusahaan.keuangan.transaksi-rekening.show', $transaksiRekening->hash_id) }}" class="inline-flex items-center px-6 py-3 border border-gray-300 rounded-xl text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
                            <i class="fas fa-times mr-2"></i>
                            Batal
                        </a>
                        <button type="submit" class="inline-flex items-center px-6 py-3 border border-transparent rounded-xl text-sm font-medium text-white bg-yellow-600 hover:bg-yellow-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-yellow-500">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function checkSaldoWarning() {
        ...
        if (selectedOption.value && jenis === 'kredit' && jumlah > 0) {
            ...
            // Add back the original transaction amount to get the actual available balance
            const originalAmount = {{ $transaksiRekening->jenis_transaksi === 'kredit' ? $transaksiRekening->jumlah : 0 }};
            const availableBalance = saldo + originalAmount;
            
            if (jumlah > availableBalance) {
                warningText.textContent = `Jumlah melebihi saldo tersedia (${mataUang} ${formatNumber(availableBalance.toString())})`;
                saldoWarning.classList.remove('hidden');
                saldoWarning.querySelector('.bg-yellow-50').className = 'bg-yellow-50 border border-yellow-200 rounded-lg p-3';
                saldoWarning.querySelector('.text-yellow-700').className = 'text-sm text-yellow-700';
            } else {
                const sisaSaldo = availableBalance - jumlah;
